/* ===== CART API ===== */
/* Fetch, add, update, remove cart items, sync, and checkout */
...
export const updateCartItemAPI = async (payload) => {
  const { data } = await axiosInstance.put('/cart', payload)
  return data
}
...
export const syncCartAPI = async (body) => {
  const { data } = await axiosInstance.post('/cart/sync', body)
  return data
}
